            ...
            describe('as original owner', () => {
                it('should fail', async () => {
                    try {
                        ...
                        assert.equal(
                            error.message,
                            'Returned error: VM Exception while processing transaction: revert Caller must be the identity owner -- Reason given: Caller must be the identity owner.'
                        )
                    }
                })
            })
            ...
            describe('as current owner', () => {
                ...
                before(async () => {
                    const sig = await signData(
                        ...
                        signerAddress,
                        privateKey,
                        Buffer.from('changeOwner').toString('hex') + stripHexPrefix(signerAddress2)
                    )
    ...
    describe('revokeDelegate()', () => {
        describe('using msg.sender', () => {
            ...
            describe('as current owner', () => {
                ...
                before(async () => {
                    previousChange = await didReg.changed(identity)
                    tx = await didReg.revokeDelegate(
                        identity,
                        '0x' + stringToBytes32('attestor'),
                        delegate3,
                        {
            ...
            describe('as current owner', () => {
                ...
                before(async () => {
                    ...
                    tx = await didReg.setAttribute(
                        identity,
                        '0x' + stringToBytes32('encryptionKey'),
                        '0x' + stringToBytes32('mykey'),
                        86400,
                        { from: owner }
                    )
                    block = await getBlock(tx.receipt.blockNumber)
                ...
                it('should create DIDAttributeChanged event', () => {
                    ...
                    assert.equal(
                        event.args.value,
                        '0x6d796b6579000000000000000000000000000000000000000000000000000000'
                    )
                    assert.equal(event.args.validTo.toNumber(), block.timestamp + 86400)
                    assert.equal(event.args.previousChange.toNumber(), previousChange.toNumber())
                })
